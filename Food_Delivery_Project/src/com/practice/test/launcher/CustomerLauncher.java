...
public class CustomerLauncher {
	...
	public static void launch() {
		...
		if (customerManager.isCustomerAlreadyRegistered(name)) {
		    ...
		    while (true) {
		        try {
		            customer = createCustomer(name); // no need to validate name again
		            customerManager.addCustomer(customer); // save to customer.txt
	...
	private static void placeOrder(Customer customer) {
		try {
			...
			// Set additional order details before saving
